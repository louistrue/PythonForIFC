...
def assign_color_to_element(ifc_file, element, color):
    ...
    styled_item = ifc_file.createIfcStyledItem(None, None, None)
    surface_style = ifc_file.createIfcSurfaceStyle("Color", "BOTH", None)
    surface_style_rendering = ifc_file.createIfcSurfaceStyleRendering(surface_style, None, None, None, None, None, None, None, "FLAT")
    ...
    # Create a representation and associate it with the element
    context = ifc_file.by_type("IfcGeometricRepresentationContext")[0]
    shape_representation = ifc_file.createIfcShapeRepresentation(context, "Body", "Tessellation", [styled_item])
    element.Representation = ifc_file.createIfcProductDefinitionShape(None, None, [shape_representation])
...
def create_shape(ifc_file, element, width, height, depth, context):
    try:
        point_list = [
            (0.0, 0.0),  # 2D Point
            (float(width), 0.0),  # 2D Point
            (float(width), float(height)),  # 2D Point
            (0.0, float(height)),  # 2D Point
            (0.0, 0.0)  # 2D Point (Closing the loop)
        ]    
...
def create_ifc(input_file, output_file):
    ...
    # Create a list to hold all the created elements
    created_elements = []
    try:
        with open(input_file, mode='r') as file:
            ...
            for row in reader:
                ...
                area = create_shape(ifc_file, element, 1.0, 1.0, 0.001, context)
